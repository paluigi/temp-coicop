...
def rac_function(modelname, test_dict, prompt_template, config_dict):
    ...
    if len(processed_items)>0:
        ...
        filename = "rac_outlines_{}_{}.csv".format(safe_modelname, datetime.now().strftime("%Y-%m-%d_%H%M%S"))
        results_df.to_csv(filename, index=False)
        minio_upload(config_dict=config_dict, filename=filename)

    if len(errors)>0:
        errors_df = pd.DataFrame(errors)
        errorname = "errors_outlines_{}_{}.csv".format(safe_modelname, datetime.now().strftime("%Y-%m-%d_%H%M%S"))
